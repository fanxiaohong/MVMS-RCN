...
class Solver_CT_FBP(object):
    def __init__(self, test_loader, args, device):
        ...
        self.theta = np.linspace(0, np.pi, int(720 / args.ds_factor), endpoint=False)
        self.theta_label = np.linspace(0, np.pi, 720, endpoint=False)

    def test(self):
        # initial test file
        result_dir_tmp = os.path.join(self.result_dir, self.CT_test_name)
        result_dir = result_dir_tmp + '_' + self.net_name + '_ds_' + str(self.ds_factor) + '/'
        ...
        with torch.no_grad():
            ...
            for data in self.test_loader:
                ...
                start = time()
                radon = Radon(full_sampled_x.shape[2], self.theta, det_count=729)
