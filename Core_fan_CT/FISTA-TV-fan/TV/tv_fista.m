function [X_out,fun_all]=tv_fista(sinogram,ds_factor, img_size, lambda,l,u,pars,D,S_D,A_R)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
X_iter= ifanbeam(sinogram,D,'FanSensorGeometry','line','FanSensorSpacing',S_D,'FanRotationIncrement',A_R*ds_factor);
...
for i=1:MAXITER
    ...
    t_old=t_new;
    % gradient step    
%     sino_pred = radon(Y, theta);
    sino_pred = fanbeam(Y,D,'FanSensorGeometry','line','FanSensorSpacing',S_D,'FanRotationIncrement',A_R*ds_factor);
   
%     sino_pred = radon(Y, theta);
%     Y_grad = iradon(sino_pred - sinogram, theta);
    Y_grad = ifanbeam(sino_pred,D,'FanSensorGeometry','line','FanSensorSpacing',S_D,'FanRotationIncrement',A_R*ds_factor);
