...
import torchvision
from data_loader_FBP_save import *
from solver_CT_FBP_fan_save import Solver_CT_FBP
###########################################################################################
# 保存 图像到图像的一一对应网络训练数据
factor_view = [64,32,16,8,4,1]
for view_i in range(len(factor_view)):
# parameter
    view_factor_i = factor_view[view_i]
    parser = ArgumentParser(description='CT-FBP')
    parser.add_argument('--net_name', type=str, default='CT-FBP-fan', help='name of net')
    ...
    parser.add_argument('--ds_factor', type=int, default=view_factor_i, help='from {32,16,8,4,2}')
    ...
    parser.add_argument('--CT_test_name', type=str, default='CT_val', help='name of CT test set')
    ...
    theta = np.linspace(0, 2*np.pi, int(1024/args.ds_factor), endpoint=False)
    #######################################################################################
